...
import { GameId, agentId, conversationId, playerId } from '../heliotrope/ids';
import { SerializedPlayer } from '../heliotrope/player';
...
export async function rememberConversation(
  ...
  const { content } = await chatCompletion(ctx,api,{
  ...
  const importance = await calculateImportance(description,ctx);
...
async function calculateImportance(description: string,ctx:ActionCtx) {
  const { content: importanceRaw } = await chatCompletion(ctx,api,{
    messages: [
      {
        role: 'user',
...
async function reflectOnMemories(
  ...
  const { content: reflection } = await chatCompletion(ctx,api,{
    messages: [
      {
        role: 'user',
        content: prompt.join('\n'),
  ...
  try {
    ...
    const memoriesToSave = await asyncMap(insights, async (item) => {
      ...
      const importance = await calculateImportance(item.insight,ctx);
