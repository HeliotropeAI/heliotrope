...
export class Game extends AbstractGame {
  ...
  tick(now: number,ctx: ActionCtx) {
    for (const player of this.world.players.values()) {
      player.tick(this, now);
    }
    for (const player of this.world.players.values()) {
      player.tickPathfinding(this, now,ctx);
    }
    for (const player of this.world.players.values()) {
  ...
  async saveStep(ctx: ActionCtx, engineUpdate: EngineUpdate): Promise<void> {
    const diff = this.takeDiff(ctx);
    await ctx.runMutation(internal.heliotrope.game.saveWorld, {
  ...
  takeDiff(ctx: ActionCtx): GameStateDiff {
    ...
    if (bufferSize > 0) {
      ...
      const content=`Packed ${Object.entries(historicalLocations).length} history buffers in ${(
          bufferSize / 1024
        ).toFixed(2)}KiB.`;
      ctx.runMutation(api.logs.createLog, { content});
      ctx.runMutation(api.logs.deleteLog,{});
    }
    this.historicalLocations.clear();
