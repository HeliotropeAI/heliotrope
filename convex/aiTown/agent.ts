...
import { MutationCtx, internalMutation, internalQuery,ActionCtx } from '../_generated/server';
...
export class Agent {
  ...
  async tick(game: Game, now: number,ctx: ActionCtx) {
    ...
    if (!conversation && !doingActivity && (!player.pathfinding || !recentlyAttemptedInvite)) {
      this.startOperation(ctx,game, now, 'agentDoSomething', {
    ...
    if (this.toRemember) {
      ...
      this.startOperation(ctx,game, now, 'agentRememberConversation', {
        worldId: game.worldId,
    ...
    if (conversation && member) {
      ...
      if (member.status.kind === 'invited') {
        ...
        if (otherPlayer.human || Math.random() < INVITE_ACCEPT_PROBABILITY) {
          ...
          // const content = `Agent ${player.id} accepting invite from ${otherPlayer.id}`;
          // await ctx.runMutation(api.logs.createLog, { content });
          // await ctx.runMutation(api.logs.deleteLog,{});
          ...
          // const content = `Agent ${player.id} rejecting invite from ${otherPlayer.id}`;
          // await ctx.runMutation(api.logs.createLog, { content });
          // await ctx.runMutation(api.logs.deleteLog,{});
      ...
      if (member.status.kind === 'walkingOver') {
        ...
        if (!player.pathfinding) {
          ...
          // const content = `Agent ${player.id} walking towards ${otherPlayer.id}...${destination}`;
          // await ctx.runMutation(api.logs.createLog, { content });
          // await ctx.runMutation(api.logs.deleteLog,{});
      ...
      if (member.status.kind === 'participating') {
        ...
        if (!conversation.lastMessage) {
          ...
          if (isInitiator || awkwardDeadline < now) {
            ...
            // const content = `${player.id} initiating conversation with ${otherPlayer.id}.`;
            // await ctx.runMutation(api.logs.createLog, { content });
            // await ctx.runMutation(api.logs.deleteLog,{});
            const messageUuid = crypto.randomUUID();
            conversation.setIsTyping(now, player, messageUuid);
            this.startOperation(ctx,game, now, 'agentGenerateMessage', {
              worldId: game.worldId,
              playerId: player.id,
              agentId: this.id,
        ...
        // const content=`${player.id} continuing conversation with ${otherPlayer.id}.`
        // await ctx.runMutation(api.logs.createLog, { content });
        // await ctx.runMutation(api.logs.deleteLog,{});
  ...
  async startOperation<Name extends keyof AgentOperations>(
    ctx: ActionCtx,
    game: Game,
    now: number,
    name: Name,
    ...
    // const content = `Agent ${this.id} starting operation ${name} (${operationId})`;
    // await ctx.runMutation(api.logs.createLog, { content });
    // await ctx.runMutation(api.logs.deleteLog,{});
...
type AgentOperations = typeof internal.heliotrope.agentOperations;

export async function runAgentOperation(ctx: MutationCtx, operation: string, args: any) {
  let reference;
  switch (operation) {
    case 'agentRememberConversation':
      reference = internal.heliotrope.agentOperations.agentRememberConversation;
      break;
    case 'agentGenerateMessage':
      reference = internal.heliotrope.agentOperations.agentGenerateMessage;
      break;
    case 'agentDoSomething':
      reference = internal.heliotrope.agentOperations.agentDoSomething;
      break;
    default:
