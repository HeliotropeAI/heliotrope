...
export async function chatCompletion(
  ctx: ActionCtx,
  api: typeof import("../../convex/_generated/api"),
...
export async function chatCompletion(
  ...
  ctx.runMutation(api.api.logs.createLog, { content:contents});
  ctx.runMutation(api.api.logs.deleteLog,{});
  const {
    result: content,
    retries,
    ms,
    ...
    if (body.stream) {
      ...
      console.log(content);
      await ctx.runMutation(api.api.logs.createLog, { content });
      await ctx.runMutation(api.api.logs.deleteLog,{});
      return content;
    }
  });
