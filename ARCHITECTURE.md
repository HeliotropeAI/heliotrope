...
This documents dives into the high-level architecture of Heliotrope and its different layers. We'll
first start with a brief overview and then go in-depth on each component. The overview should
be sufficient for forking Heliotrope and changing game or agent behavior. Read on to the deep dives
...
Heliotrope is split into a few layers:

- The server-side game logic in `convex/heliotrope`: This layer defines what state Heliotrope maintains,
...
- The client-side game UI in `src/`: Heliotrope uses `pixi-react` to render the game state to the
  browser for human consumption.
- The game engine in `convex/engine`: To make it easy to hack on the game rules, we've separated
  out the game engine from the Heliotrope-specific game rules. The game engine is responsible for
...
for the async work, and `convex/heliotrope/agent.ts` for the game loop logic.
If you would like to add new gameplay elements (that both humans and agents can interact with), add
the feature to `convex/heliotrope`, render it in the UI in `src/`, and respond to it in `convex/heliotrope/agent.ts`.
...
## Heliotrope game logic (`convex/Heliotrope`)

### Data model

Heliotrope's data model has a few concepts:

- Worlds (`convex/heliotrope/world.ts`) represent a map with many players interacting together.
- Players (`convex/heliotrope/player.ts`) are the core characters in the game. Players have human readable names and
...
- Conversations (`convex/heliotrope/conversations.ts`) are created by a player and end at some point in time.
- Conversation memberships (`convex/heliotrope/conversationMembership.ts`) indicate that a player is a member
...
2. Game tables (`convex/heliotrope/schema.ts`) for game state. To keep game state small and efficient to
   read and write, we store Heliotrope's data model across a few tables. See `convex/heliotrope/schema.ts` for an overview.
...
### Inputs (`convex/heliotrope/inputs.ts`)

Heliotrope modifies its data model by processing inputs. Inputs are submitted by players and agents and
processed by the game engine. We specify inputs in the `inputs` object in `convex/heliotrope/inputs.ts`.
...
- Moving a player to a particular location (`moveTo`): Movement in Heliotrope is similar to RTS games, where
...
- Agent inputs are imported from `heliotrope/agentInputs.ts` for things like remembering conversations,
...
Given the description of Heliotrope's game behavior in the previous section,
...
Heliotrope's game behavior is implemented in the `Game` subclass.
...
`Game` provides an abstract method `handleInput` that `heliotrope` implements with its specific behavior.
...
- Ticks are run at a high frequency, configurable with `tickDuration` (milliseconds). Since Heliotrope has smooth motion
...
- It's generally a good idea to break up game logic into separate systems that can be ticked forward independently.
  For example, Heliotrope's `tick` method advances pathfinding with `Player.tickPathfinding`, player positions with
...
many ticks into a _step_. Heliotrope runs steps at only 1 time per second. Here's how a step works:
...
1. The Convex scheduler calls the `convex/heliotrope/main.ts:runStep` action.
2. The `runStep` action calls `convex/heliotrope/game.ts:loadWorld` to load the current game state. This query calls
...
5. At the end of a step, the framework calls `Game.saveStep`, which computes a diff of the game state since
   the beginning of the step and passes the diff to the `convex/heliotrope/game.ts:saveWorld` mutation.
...
One guiding principle for Heliotrope's architecture is to keep the usage as close to "regular Convex" usage as possible. So,
...
6. Inputs are defined with `inputHandler` and are given an instance of the heliotrope game to modify,
...
Heliotrope's game engine has a few design goals:
...
- The game engine is designed to be single threaded. JavaScript operating over plain objects
  ...
  may not be a good fit on Heliotrope's engine today.
